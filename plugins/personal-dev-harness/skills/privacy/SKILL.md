---
description: Change harness privacy mode for this Claude Code session. Use when asked to make work private, confidential, isolated, normal again, or to prevent the harness from learning from a task.
---

Use `harness-memory session-mode --session "$CLAUDE_CODE_SESSION_ID" --cwd "$PWD" --mode <normal|private|isolated>`.
