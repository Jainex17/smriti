#!/usr/bin/env python3
"""Keep only session metadata; never copy the transcript into smriti storage."""
...
def main() -> int:
    try:
        ...
        return 0
    # Session learning is written explicitly by the smriti agent only after verified work.
    ...
    # Private/isolated sessions never mutate smriti memory.
    try:
        root = os.environ.get("CLAUDE_PLUGIN_ROOT") or str(Path(__file__).resolve().parents[1])
        memory = Path(root) / "scripts" / "smriti_memory.py"
